...
static double log_gamma_windschitl(double x)
{
  return 0.918938533204673 + (x-0.5)*log(x) - x
         + 0.5*x*log( x*sinh(1/x) + 1/(810.0*pow(x, (double)(6.0))) );
}
    ...
    if( used->data[ list_p->x + list_p->y * used->xsize ] == NOTUSED &&
          ...
          because we are only interested in the exact NOTDEF value */
      {
        ...
        if (rects_a <= rects_u) {
            ...
            rects_a *= 2;
			rects = (struct rect*)realloc(rects, rects_a * sizeof(rects[0]));
        }
        rects[rects_u++] = rec;
                           ...
                           int ** reg_img, int * reg_x, int * reg_y )
{
  ...
  int need_to_union = 1;    /* Bool if you need to post procces image by 
  ...
  double dist_threshold = 2;   /* Maximum distance between two line which 
                                   we would union                             */
...
{
  /* LSD parameters */
  double scale = 0.8;       /* 0.8 Scale the image by Gaussian filter to 'scale'. */
