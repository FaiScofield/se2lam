...
//        mNumValidMatches = mvMatches.size();
    ...
    gms_matcher(const vector<KeyPoint> &vkp1, const Size size1, const vector<KeyPoint> &vkp2, const Size size2, const vector<int> &vMatches12)
    {
        // Input initialize
        NormalizePoints(vkp1, size1, mvP1);
        NormalizePoints(vkp2, size2, mvP2);
        mNumberMatches = vMatches12.size();
        ConvertMatches(vMatches12, mvMatches);
//        mNumValidMatches = mvMatches.size();
...
private:
    ...
    void NormalizePoints(const vector<Point2f> &kp, const Size &size, vector<Point2f> &npts)
    {
        const size_t numP = kp.size();
        const int width = size.width;
        const int height = size.height;
    ...
    void ConvertMatches(const vector<int> &vMatches12, vector<pair<int, int>> &vMatches)
    {
        vMatches.resize(mNumberMatches);
        for (size_t i = 0; i < mNumberMatches; i++)
        {
            vMatches[i] = pair<int, int>(i, vMatches12[i]);
        }
    }
...
int gms_matcher::run(int RotationType)
{
    ...
    mvMatchPairs.assign(mNumberMatches, pair<int, int>(-1, -1));
