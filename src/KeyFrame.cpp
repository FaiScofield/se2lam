...
KeyFrame::KeyFrame(const Frame& frame) : Frame(frame), mbBowVecExist(false), mbNull(false)
{
    ...
    mViewMPsInfo =
        vector<Eigen::Matrix3d, Eigen::aligned_allocator<Eigen::Matrix3d>>(sz, Eigen::Matrix3d::Identity() * -1);
    mNextIdKF++;
    mIdKF = mNextIdKF;
...
void KeyFrame::setViewMP(const Point3f& pt3f, int idx, const Eigen::Matrix3d& info)
{
    locker lock(mMutexObs);
    mViewMPs[idx] = pt3f;
...
void KeyFrame::addObservation(const PtrMapPoint& pMP, int idx)
{
    locker lock(mMutexObs);
    if (pMP->isNull())
        return;
