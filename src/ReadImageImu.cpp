...
//图像平滑滤波
void ReadImageImu::smooth_image_float(const Mat& img, Mat& gimgf, int w)
{
    Mat gimg;
    if (img.channels() == 3) {
    ...
    GaussianBlur(img, gimgf, Size(3, 3), w);
    return;
}


//根据IMU数据计算相邻两帧的夹角
bool ReadImageImu::compute_theta_IMU(int ptime, int ctime, const string& fname,
                                     const string& shape_fname, Out_IMU_Data& outImuData)
{
    static bool b_open = false;
    static int index = 0;
    ...
    if (!b_open) {
        ...
        //只用了Cw,没有使用Ca;
        for (int i = 0; i < 3; i++)
            fin_sh >> bias_gyro[i];  //陀螺仪偏差
        for (int k = 0; k < 3; k++)
            for (int i = 0; i < 3; i++)
                fin_sh >> Cw[k][i];  // shape matrix
        ...
        while (!fin.eof()) {
            ...
            for (int i = 0; i < 3; i++)
                a[i] = a[i] - bias_acc[i];  //未使用
            for (int i = 0; i < 3; i++)
                w[i] = w[i] - bias_gyro[i];  //使用z-v
            // w[2] = w[2] * 180 / 3.14;
            ...
            m.rate[0] =
                Cw[0][0] * w[0] + Cw[0][1] * w[1] + Cw[0][2] * w[2];  // S转置的逆*（z-v）=Rt_t+1
    ...
    for (tt = ptime; tt < ctime;) {
        ...
        double w[3], x, y;
        for (int i = 0; i < 3; i++)
            w[i] = a * vdata[index].rate[i] + (1 - a) * vdata[index + 1].rate[i];
