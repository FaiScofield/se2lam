      ...
      mnLostFrames(0), mbFinishRequested(false), mbFinished(false)
{
    ...
    mK = Config::Kcam;
    mD = Config::Dcam;
    mHomography = Mat::eye(3, 3, CV_64FC1);  // double
        ...
        if (sensorUpdated) {
            ...
            mpSensors->readData(odo, img, Imu_theta, useCeil);
            {
                ...
                if (mState == cvu::FIRST_FRAME) {
                    // 为初始帧创建帧信息true为分块模式，false为全局模式
                    createFrameFirstKlt(img, odo, useCeil);
                } else {
                    if (useCeil)
                        // 非初始帧则进行tracking,计算位姿trackKltCeil为分块模式，trackKlt为全局模式
                        trackKltCeil(img, odo, Imu_theta);
                    else
                        // 非初始帧则进行tracking,计算位姿trackKltCeil为分块模式，trackKlt为全局模式
                        trackKlt(img, odo, Imu_theta);
                }
            }
 ...
 * 2019.10.28
 */
void TrackKlt::createFrameFirstKlt(const Mat& img, const Se2& odo, bool ceil)
{
...
void TrackKlt::trackKlt(const Mat& img, const Se2& odo, double imuTheta)
{
    ...
    if (mvCurPts.size() > 0) {
        vector<uchar> status;
        vector<float> err;
        cv::calcOpticalFlowPyrLK(rot_img, mForwImg, points_prev, mvForwPts, status, err,
                                 cv::Size(21, 21), 0);
    ...
    if (needPoints > 50) {
        ...
        cv::goodFeaturesToTrack(imgGray, mvNewPts, needPoints, 0.01, mnMinDist, mMask);
    } else
        mvNewPts.clear();
    ...
    cout << "[Track] ORBmatcher get valid/tatal matches " << mnInliers << "/" << mRef_kpt << endl;
    updateFramePose();
    if (mnInliers > 10) {  // 内点数大于10则三角化计算MP
        mnTrackedOld = doTriangulate();  // 更新viewMPs
        drawMatchesForPub(true);
        if (mbPrint)
            printf("[Track][Info ] #%ld 与参考帧匹配获得的点数: trackedOld/inliers/matchedSum = "
                   "%d/%d/%d.\n",
...
void TrackKlt::trackKltCeil(const Mat& img, const se2lam::Se2& odo, double imuTheta)
{
    ...
    Mat maskline = getLineMask(imgGray, false);
    mMask = mMask.mul(maskline);
    detectFeaturePointsCeil(imgGray, mMask);
...
bool TrackKlt::inBorder(const cv::Point2f& pt)
{
    ...
    return BORDER_SIZE <= img_x && img_x < mnImgCols - BORDER_SIZE && BORDER_SIZE <= img_y &&
           img_y < mnImgRows - BORDER_SIZE;
}
...
void TrackKlt::setMask()
{
    ...
    sort(cnt_pts_id.begin(), cnt_pts_id.end(),
         [](const pair<int, pair_mask>& a, const pair<int, pair_mask>& b) {
 ...
 * 2019.10.28
 */
void TrackKlt::getRotatePoint(vector<cv::Point2f> Points, vector<cv::Point2f>& dstPoints,
                              const cv::Point rotate_center, double angle)
{
    cv::Point2f prpoint;
    double x1 = 0, y1 = 0;
...
void TrackKlt::detectFeaturePointsCeil(cv::Mat frame, cv::Mat mask)
{
    ...
    for (int i = 0; i < 16; i++) {
        ...
        if (dceilNum > 10) {
            cv::goodFeaturesToTrack(ceil_Image[i], ceilFeature[i], dceilNum, 0.01, mnMinDist,
                                    ceil_mask[i], 3);
...
void TrackKlt::setMaskCeil()
{
    ...
    sort(cnt_pts_id.begin(), cnt_pts_id.end(),
         [](const pair<int, pair_mask_ceil>& a, const pair<int, pair_mask_ceil>& b) {
             return a.first > b.first;
         });
    mvForwPts.clear();
    mvIds.clear();
